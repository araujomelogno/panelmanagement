...
public class VincularPropiedadesDialog extends Dialog {
	...
	private void setupGrid() {
		Grid.Column<PanelistProperty> nameColumn = grid.addColumn(PanelistProperty::getName).setHeader("Nombre")
		...
		grid.addComponentColumn(property -> {
			...
			Optional<SurveyPropertyMatching> match = surveyPropertyMatchingService.findBySurveyAndProperty(survey,
					property);
...
//        grid.addColumn(PanelistProperty::getName).setHeader("Nombre").setSortable(true).setFlexGrow(1);
		...
		grid.getHeaderRows().clear();
		Grid.HeaderRow headerRow = grid.appendHeaderRow();
		headerRow.getCell(nameColumn).setComponent(nameFilter);
	...
	private void save() {
		for (Map.Entry<PanelistProperty, Checkbox> entry : checkboxMap.entrySet()) {
			...
			if (checkbox.getValue()) {
				Optional<SurveyPropertyMatching> existingMatch = surveyPropertyMatchingService
						.findBySurveyAndProperty(survey, property);
