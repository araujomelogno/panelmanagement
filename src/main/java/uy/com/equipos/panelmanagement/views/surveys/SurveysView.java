...
public class SurveysView extends Div implements BeforeEnterObserver {
    ...
    private void sendReminderAction() {
        ...
        for (SurveyPanelistParticipation participation : participations) {
            ...
            if (panelist != null) {
                MessageTask mt = new MessageTask();
                mt.setJobType(JobType.ALCHEMER_REMINDER); // Key difference: ALCHEMER_REMINDER
                mt.setCreated(LocalDateTime.now());
                mt.setStatus(MessageTaskStatus.PENDING);
                mt.setSurvey(currentSurveyWithParticipations);
                mt.setPanelist(panelist);
        ...
        Notification.show(tasksCreated + " tareas de recordatorio creadas para los panelistas.", 3000, Notification.Position.BOTTOM_START);
    }
